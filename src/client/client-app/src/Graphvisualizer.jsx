...
//       chartData.labels = sensors.readings.map(item => item.timestamp);
...
const GraphVisualizer = ({ selectedRun, sliderValue }) => {
  ...
  useEffect(() => {
    ...
    selectedRun.data.forEach(sensors => {
      ...
      // Apply downsampling to the data arrays
      const threshold = 1000; // Set the threshold for the maximum number of data points
      dataArrayX = downsample(dataArrayX, threshold);
      dataArrayY = downsample(dataArrayY, threshold);
      dataArrayZ = downsample(dataArrayZ, threshold);
      chartData.labels = downsample(chartData.labels, threshold);
