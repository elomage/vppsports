...
/**
 * Is used to buffer writes to the SD card for shorter write times
 */
class SDCardBuffer {
...
public:
	 ...
	 * @return True, if data is appended, false if the append would cause an overrun
	 */
...
class SDHandler {
	 ...
	 */
	static bool _RecursivelyDeleteIfExists(const TCHAR *path);
	...
	/**
	 * Appends the given sensor log buffer to the appropriate log file
	 *
	 * @param filename The sensor filename as generated by _GetSensorLogFilePath
	 * @param logBuffer The appropriate buffer object
...
public:
	...
	/**
	 * Checks if a config is localy stored
	 *
	 * @return True, if the config is stored on the SD card
	 */
	 ...
	 *
	 * return Localy stored settings object
	 */
